...
const protectMentor = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) //token is in headers and starts with Bearer token
    {
        ...
        try {
            ...
            const decoded = jwt.verify(token, jwt_secret);
            const email = decoded.id
            var query = `select Id from Mentor where Email='${email}'`
            var [results, fields] = await connection.execute(query);
            if (results?.length>0) {
                req.user = results[0]["Id"]
                next();
...
const protectAdmin = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) //token is in headers and starts with Bearer token
    {
        ...
        try {
            ...
            const decoded = jwt.verify(token, jwt_secret);
            const email = decoded.id
            var query = `select Id from Admin where Email='${email}'`
            var [results, fields] = await connection.execute(query);
            if (results?.length>0) {
                req.user = results[0]["Id"]
                next();
...
module.exports = { protectMentee, protectMentor, protectAdmin}